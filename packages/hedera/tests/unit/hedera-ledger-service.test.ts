...
import {
    ...
    RegisterRevocationStatusListOptions, RegisterSchemaOptions
...
import {DID_ROOT_KEY_ID, parseDID, Publisher} from '@hiero-did-sdk/core'
...
describe('HederaLedgerService', () => {
  ...
  beforeEach(() => {
    ...
    mockedGenerateDeactivateDIDRequest = generateDeactivateDIDRequest as jest.MockedFunction<typeof generateDeactivateDIDRequest>
    ...
    service = new HederaLedgerService({
      ...
      jest.spyOn(service['clientService'], 'withClient').mockImplementation(async (props, operation) => {
  ...
  describe('createDid', () => {
    it('should creates DID without didDocument', async () => {
      ...
      (generateCreateDIDRequest as jest.Mock).mockResolvedValue({ state: {}, signingRequest: { serializedPayload: new Uint8Array() } });
    ...
    it('should creates DID with didDocument and calls updateDid', async () => {
      ...
      const updateDidSpy = jest.spyOn(service, 'updateDid').mockResolvedValue({ did: 'did:hedera:1234', someProp: true } as any);
  ...
  describe('updateDid', () => {
    ...
    it('should performs update flow successfully', async () => {
      const keys: DidDocumentKey[] = [{ kmsKeyId, didDocumentRelativeKeyId: DID_ROOT_KEY_ID }, { kmsKeyId: 'some-key', didDocumentRelativeKeyId: '#abc' }];
  ...
  describe('deactivateDid', () => {
    ...
    it('should throws an error if root key is not found in deactivateDid', async () => {
      ...
      await expect(service.deactivateDid(mockAgentContext as AgentContext, props))
          .rejects.toThrow('The root key not found in the KMS')
    })
