...
import type { ICredDef, ICredDefPrimary, ICredDefRevocation } from '../ts/src/cardano/models/ICredDef'
import type { IRevReg } from '../ts/src/cardano/models/IRevReg'
import type { ISchema } from '../ts/src/cardano/models/ISchema'
import type {
  AnonCredsRegistry,
  GetSchemaReturn,
...
export class CardanoAnonCredsRegistry implements AnonCredsRegistry {
  ...
  public constructor({
    ...
    this.cardano = new Cardano(blockfrostProjectId, cardanoNerwork, cardanoAddressCborHex)
  }
  ...
  public async registerSchema(
    ...
    try {
      ...
      const signature = await this.signAnoncredObject(agentContext, options.schema.issuerId, message)
      if (signature !== null) {
  ...
  public async registerCredentialDefinition(
    ...
    try {
      ...
      const signature = await this.signAnoncredObject(agentContext, options.credentialDefinition.issuerId, message)
      if (signature !== null) {
  ...
  private async verifySignature(
    ...
    const didKey = new Key(publicKeyBuffer, KeyType.Ed25519)
  ...
  private async signAnoncredObject(agentContext: AgentContext, did: string, data: string): Promise<string | null> {
    const didResolver = agentContext.dependencyManager.resolve(DidResolverService)
    const didDoc = await didResolver.resolve(agentContext, did)
    if (didDoc.didDocument == null) {
      return null
